#include "InterruptHandler.h"
	...
	if (interrupt_type == PIN_INTERRUPT) {
		if (trigger_type == PIN_LOW_INTERRUPT || trigger_type == PIN_FALL_INTERRUPT) {
	...
	bool InterruptHandler::Check_for_interrupt(){
		...
		switch(interrupt_type){
			...
			case PIN_INTERRUPT :
				switch (trigger_type) {
				case PIN_FALL_INTERRUPT:
					if (previous_pin_state && digitalRead(input) == LOW) {
						previous_pin_state = digitalRead(input) == HIGH;
						return(true);
					}
					else {
						previous_pin_state = digitalRead(input) == HIGH;
					...
					break;
				case PIN_RISE_INTERRUPT:
					if (!previous_pin_state && digitalRead(input) == HIGH) {
						previous_pin_state = digitalRead(input) == HIGH;
						return(true);
					}
					else {
					...
					break;
				case PIN_HIGH_INTERRUPT:
					return(digitalRead(input) == HIGH);
					break;
				case PIN_LOW_INTERRUPT:
