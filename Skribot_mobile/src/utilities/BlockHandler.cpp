  ...
  return(false);
   ...
   bool BlockHandler::addConst(int id){
    ConstBlock *cblock = new ConstBlock(id);
    if(blockList_N < blockList_MAX){
      blockList[blockList_N] = cblock;
      blockList_N++;
      return(true);
	...
	bool BlockHandler::MakeConections(){
      ...
      Serial.println("Logic OK");
  		...
  		for(int kk = 0 ; kk < IfblockList_N ; kk++){
  			if(!IfblockList[kk]->set_logics(blockList,blockList_N,blockList,blockList_N))return(false);
  		}
    #ifdef DEBUG_MODE
    ...
    case 'L':
     ...
     case 'C':
          ...
          if(AllMessage[Mcursor] == 'I'){
            ...
            value = 0;
            Mcursor += 2;
          }else if(AllMessage[Mcursor] == 'B'){
            Mcursor += 2;
            memoryOk = addConst(id);
            break;
          }else{
