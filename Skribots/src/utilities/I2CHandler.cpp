    ...
    byte I2CHandler::I2CRecive(byte addr,byte size){
	  ...
	  bool I2CHandler::I2CTransfer(byte *msg,byte addr){
           for(byte p = 0; p < 15; p++)output_buffer[p] = 0;
              byte Nrec = msg[0] & B00001111;              //number of bytes to send
              byte Nsend  = (msg[0]>>4) & B00001111;         // nuber of bytes to recive
              byte output[Nrec];
              i2c->beginTransmission(addr);           //transfering command address
              ...
              if(Nrec != 0){
                  i2c->requestFrom((int)addr,(int)Nrec,1);
                for(byte zz = 0; zz<Nrec;zz++){
                  while(i2c->available() == 0){
                  }
                  output[zz] = i2c->read();             //receiving data 
                }
