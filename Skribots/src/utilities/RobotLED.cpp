...
#define LED_COMMAND_OFFSET 20
	...
	RobotLED::RobotLED(int pin,String _name,byte N_LED){
		//pixel = new Adafruit_Neo//pixel(N_LED, pin, NEO_GRB + NEO_KHZ800);
        name = _name;
        nLED = N_LED;
        id = 99;
        //pixel->begin();
    ...
    RobotLED::RobotLED(byte pin,byte _id,byte N_LED){
        //pixel = new Adafruit_Neo//pixel(N_LED, pin, NEO_GRB + NEO_KHZ800);
        id = _id;
        nLED = N_LED;
        name = "none";
    ...
    void RobotLED::turnON(int R, int G, int B,byte N_LED){
         ...
         ////pixel->show(); 
          if(N_LED == 0){
            //for(int yy = 0; yy < nLED; yy++)//pixel->set//pixelColor(yy, //pixel->Color(R,G,B));
          }else{ 
          //pixel->set//pixelColor(N_LED-1, //pixel->Color(R,G,B)); 
          }
