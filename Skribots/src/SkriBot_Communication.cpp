#include "Skribot_main.h"
 ...
 void Skribot::SendMessage(float info){
   Serial.print(info);
   delay(10);
 }

  void Skribot::SendMessage(double info){
   Serial.print(info);
   delay(10);
 }

  void Skribot::SendMessageLine(float info){
   Serial.print(info);
   delay(10);
 }

  void Skribot::SendMessageLine(double info){
   Serial.print(info);
   delay(10);
 }
#endif
void Skribot::IgnoreCONBRK(){
  ...
  void Skribot::BLE_write(char const *msg){
    BTmodule->BLE_write((char*)msg);
  }
...
if (claw_closed && (millis() - claw_update_time > 3000)) {
    Claws[0]->updateClaw();
    delay(20);
    Claws[0]->updateClaw();
